...
12. [Как выложить проект на GitHub](#как-выложить-проект-на-github)
...
1. Зарегистрируйтесь на <https://github.com> и нажмите **New repository**. Придумайте имя, выберите
   Public или Private и **не** ставьте галочки «Add a README / .gitignore / license»: эти файлы уже
   есть в проекте.
2. В терминале, в папке проекта, представьтесь Git (один раз на компьютер):
...
3. Создайте первую версию и отправьте её:
   ...
   Перед `git commit` посмотрите вывод `git status`: там **не должно быть** `.env`, `storage/`,
   `archive/` — они исключены файлом `.gitignore`.
4. При `git push` откроется окно входа в GitHub. На Windows его показывает Git Credential Manager,
   на macOS можно войти так: `brew install gh && gh auth login`. Обычный пароль GitHub в терминале
   не принимается, нужен вход через браузер или токен (Settings → Developer settings → Personal
   access tokens).
5. Следующие изменения отправляются так:
