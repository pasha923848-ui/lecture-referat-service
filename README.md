...
перечисляет видеолекции и одной строкой указывает, какой ИИ-сервис использовался. Полное описание
работы программы со всеми промтами не попадает в сам реферат, а сохраняется рядом с ним в файле
`ЛК<номер>_Фамилия_группа.ИИ-сервис.txt` — его можно приложить или показать, если преподаватель
спросит подробности. Этого требуют правила оформления рефератов: «если использован ИИ-сервис, то
указать какой промт и какой сервис». Строку про ИИ-сервис не удаляйте, а сам список источников
имеет смысл доработать вручную — добавить учебники и статьи, которыми вы пользовались. Если сдать
сгенерированный текст как полностью свой и скрыть использование ИИ, это нарушение академической
честности, и отвечать за него будете вы.
...
Последняя страница реферата — список источников: видеолекции и одна строка с названием ИИ-сервиса.
Дополните его своими источниками (учебники, ГОСТы, статьи) перед сдачей.
